...
const UpdateTagsDialog = ({ open, onClose, tag }) => {
  ...
  // Update initial title and color based on the tag prop
  useEffect(() => {
    if (tag) {
      setFormData({
        title: tag?.title || "",
      ...
      const selectedColor = colorSet.find((col) => col.title === tag?.txColor);
      if (selectedColor) {
  ...
  const handleUpdate = async () => {
    try {
      await TagManagement.updateTag(tag._id, formData);
      onClose();
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
    >
      <CloseDialog title="Update Tag" handleClose={onClose} />
...
export default UpdateTagsDialog;
